<div class="page-header mb-4 d-flex justify-content-between align-items-center"
    style="padding-bottom: 1.25rem; border-bottom: 1px solid var(--border);">
    <div>
...
<?php if (empty($clearances)): ?>
    <div
        style="background: var(--surface); border: 1px solid var(--border); border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); padding: 3rem; text-align: center;">
...
<?php else: ?>
    <div
        style="background: var(--surface); border: 1px solid var(--border); border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); overflow: hidden;">
            ...
            <table class="data-table" style="width: 100%; font-size: 0.86rem;">
                ...
                <tbody>
                    <?php foreach ($clearances as $c): ?>
                        <tr class="border-bottom" style="transition: background .15s;"
                            onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background=''">
                            <td data-label="Clearance Name">
                                <strong><?= htmlspecialchars($c['name']) ?></strong>
                                <?php if (!empty($c['description'])): ?>
                                    <br><small class="text-muted"
                                        style="font-size:0.78rem;"><?= htmlspecialchars($c['description']) ?></small>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="text-center" data-label="School Year" style="white-space:nowrap;">
                                <?= htmlspecialchars($c['school_year']) ?></td>
                            <td class="text-center" data-label="Signatories"><span
                                    class="badge badge-info"><?= $c['signatory_count'] ?></span></td>
                            <td class="text-center" data-label="Enrollment Committee"><span
                                    class="badge badge-info"><?= $c['enrollment_committee_count'] ?></span></td>
                            <td class="text-center" data-label="Students"><span
                                    class="badge badge-info"><?= $c['student_count'] ?></span></td>
                            <td class="text-end" data-label="Actions" style="white-space: nowrap;">
                                <div class="action-cell">
                                    ...
                                    <form action="<?= BASE_URL ?>admin/clearances/archive" method="POST"
...
<div id="createClearanceModal" class="modal" style="display:none;">
    <div class="modal-box">
        <div class="modal-header">
            <h3>New Clearance</h3>
            <button onclick="document.getElementById('createClearanceModal').style.display='none'"
        ...
        <form action="<?= BASE_URL ?>admin/clearances/create" method="POST" class="modal-form">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" required placeholder="e.g. 1st Semester Clearance">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div style="display:flex; flex-direction:row; gap:1.5rem; align-items:center; margin-top:.25rem;">
                    <label
                        style="display:inline-flex; align-items:center; gap:.4rem; cursor:pointer; font-size:.9rem; font-weight:500; color:var(--text); text-transform:none; letter-spacing:normal; width:auto;">
                        <input type="radio" name="type" value="regular"
                            style="width:16px; height:16px; margin:0; padding:0; flex-shrink:0; accent-color:var(--primary); cursor:pointer;"
                            checked> Semestral Clearance
                    </label>
                    <label
                        style="display:inline-flex; align-items:center; gap:.4rem; cursor:pointer; font-size:.9rem; font-weight:500; color:var(--text); text-transform:none; letter-spacing:normal; width:auto;">
                        <input type="radio" name="type" value="exit"
                            style="width:16px; height:16px; margin:0; padding:0; flex-shrink:0; accent-color:var(--primary); cursor:pointer;">
                        Exit Clearance
                    </label>
                </div>
...
<div id="editClearanceModal" class="modal" style="display:none;">
    <div class="modal-box">
        <div class="modal-header">
            ...
            <button onclick="document.getElementById('editClearanceModal').style.display='none'"
                class="close-btn">✕</button>
        </div>
        <form action="<?= BASE_URL ?>admin/clearances/edit" method="POST" class="modal-form">
            ...
            <div class="form-group">
                ...
                <div style="display:flex; flex-direction:row; gap:1.5rem; align-items:center; margin-top:.25rem;">
                    <label
                        style="display:inline-flex; align-items:center; gap:.4rem; cursor:pointer; font-size:.9rem; font-weight:500; color:var(--text); text-transform:none; letter-spacing:normal; width:auto;">
                        <input type="radio" name="type" id="editTypeRegular" value="regular"
                            style="width:16px; height:16px; margin:0; padding:0; flex-shrink:0; accent-color:var(--primary); cursor:pointer;">
                        Semestral Clearance
                    </label>
                    <label
                        style="display:inline-flex; align-items:center; gap:.4rem; cursor:pointer; font-size:.9rem; font-weight:500; color:var(--text); text-transform:none; letter-spacing:normal; width:auto;">
                        <input type="radio" name="type" id="editTypeExit" value="exit"
                            style="width:16px; height:16px; margin:0; padding:0; flex-shrink:0; accent-color:var(--primary); cursor:pointer;">
                        Exit Clearance
                    </label>
                </div>
